...
In I<Compiz> the workspaces are arranged into a grid, so each workspace has a
pair of ``coordinates'' x,y. To switch to a given workspace in a B<--compiz>
...
I recommend creating a keyboard shortcut for  B<wmjump> (or B<wmjump --compiz>
...
I tested it on I<Xfce> (Xubuntu), I<gnome> I<Metacity> (Ubuntu, Fedora),
I<gnome> I<Compiz> (see OPTION B<--compiz>), I<Kwin> (Kubuntu) and I<StumpWM>
(v0.9.5 on Intrepid, see special instructions for I<StumpWM> in the SETUP
section). Works with I<xmonad>, see the xmonad section in the wiki.  
...
Use this option if you are on I<Compiz> window manager (``wall'').  The I<Compiz>
...
For I<xmonad> users, see the wiki page C<http://code.google.com/p/wmjump/wiki/xmonad>
